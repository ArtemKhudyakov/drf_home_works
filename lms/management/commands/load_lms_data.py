...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--noinput", action="store_true", help="Пропустить подтверждение")

    def handle(self, *args, **options):
        if not options["noinput"]:
            confirm = input("⚠️  Вы уверены? Это удалит все существующие курсы и уроки. (y/N): ")
            if confirm.lower() != "y":
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"📊 Загружено: {course_count} курсов, {lesson_count} уроков"))

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"❌ Ошибка загрузки фикстур: {e}"))
            # Если файл не найден, предлагаем создать его
            if "No such file or directory" in str(e):
                self.stdout.write(self.style.NOTICE("💡 Создайте фикстуру командой:"))
                self.stdout.write(
